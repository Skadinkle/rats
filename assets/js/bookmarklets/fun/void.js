			...
			if (element !== spreadElement && !isAncestor(spreadElement, element)) {
				...
				if (boundsOverlap(spreadBounds, elementBounds)) {
					if (element.tagName === "IMG") {
						...
						element.style.textShadow = "0 0 3px rgba(255,255,255,.5)";
					}
				}
			}
		}
		if (x < -spreadRadius || x > window.innerWidth + spreadRadius || y < -spreadRadius || y > window.innerHeight + spreadRadius) {
