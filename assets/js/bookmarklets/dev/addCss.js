javascript:(function addcss(document, s) {
	...
	function getActiveSelection(doc) {
		if (arguments.length === 0) doc = document;
		if (!doc || typeof doc.getSelection !== "function") return "";
		if (!doc.activeElement) return `${doc.getSelection()}`;
		var activeElement = doc.activeElement;
		try {
			if (typeof activeElement.contentDocument === "object" && activeElement.contentDocument !== null) return getActiveSelection(activeElement.contentDocument);
		}
		catch (e) {
			return `${doc.getSelection()}`;
		}
		if (typeof activeElement.value === "string") {
			if (activeElement.selectionStart !== activeElement.selectionEnd) return activeElement.value.substring(activeElement.selectionStart, activeElement.selectionEnd);
